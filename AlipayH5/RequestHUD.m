...
@interface RequestHUD ()
@property (nonatomic, readwrite) RequestHUDMaskType maskType;
...
- (void)showWithStatus:(NSString*)string maskType:(RequestHUDMaskType)hudMaskType networkIndicator:(BOOL)show;
...
+ (RequestHUD *)sharedView
{
    static dispatch_once_t once;
    static RequestHUD *sharedView;
    dispatch_once(&once, ^ { sharedView = [[RequestHUD alloc] initWithFrame:[[UIScreen mainScreen] bounds]]; });
    return sharedView;
}
...
+ (void)setStatus:(NSString *)string
{
    [[RequestHUD sharedView] setStatus:string];
}
...
+ (void)show
{
    [[RequestHUD sharedView] showWithStatus:nil maskType:RequestHUDMaskTypeNone networkIndicator:NO];
}

+ (void)showWithStatus:(NSString *)status
{
    [[RequestHUD sharedView] showWithStatus:status maskType:RequestHUDMaskTypeNone networkIndicator:NO];
}

+ (void)showWithMaskType:(RequestHUDMaskType)maskType
{
    [[RequestHUD sharedView] showWithStatus:nil maskType:maskType networkIndicator:NO];
}

+ (void)showWithStatus:(NSString*)status maskType:(RequestHUDMaskType)maskType
{
    [[RequestHUD sharedView] showWithStatus:status maskType:maskType networkIndicator:NO];
}

+ (void)showErrorWithStatus:(NSString *)string
{
    [RequestHUD showErrorWithStatus:string duration:2];
}

+ (void)showErrorWithStatus:(NSString *)string duration:(NSTimeInterval)duration
{
    [RequestHUD show];
    [RequestHUD dismissWithError:string afterDelay:duration];
}
...
+ (void)dismiss
{
    [[RequestHUD sharedView] dismiss];
}

+ (void)dismissWithSuccess:(NSString*)successString
{
    [[RequestHUD sharedView] dismissWithStatus:successString error:NO];
}

+ (void)dismissWithSuccess:(NSString *)successString afterDelay:(NSTimeInterval)seconds
{
    [[RequestHUD sharedView] dismissWithStatus:successString error:NO afterDelay:seconds];
}

+ (void)dismissWithError:(NSString*)errorString
{
    [[RequestHUD sharedView] dismissWithStatus:errorString error:YES];
}

+ (void)dismissWithError:(NSString *)errorString afterDelay:(NSTimeInterval)seconds
{
    [[RequestHUD sharedView] dismissWithStatus:errorString error:YES afterDelay:seconds];
}
...
+ (void)dismissWithError:(NSString *)successString title:(NSString *)titleString andImage:(UIImage *)image afterDelay:(NSTimeInterval)duration {
    [[RequestHUD sharedView] dismissWithStatus:successString title:titleString Image:image afterDelay:duration];
}
...
- (void)drawRect:(CGRect)rect
{
    ...
    switch (self.maskType)
    {
        ...
        case RequestHUDMaskTypeGradient:
        {
...
- (void)showWithStatus:(NSString*)string maskType:(RequestHUDMaskType)hudMaskType networkIndicator:(BOOL)show
{
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if(self.maskType != RequestHUDMaskTypeNone)
        {
...
+ (BOOL)isVisible
{
    return ([RequestHUD sharedView].alpha == 1);
}
